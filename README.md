...
사용자는 직접 `식사 유형(아침/점심/저녁/간식)`, `메뉴명`, `탄단지(탄수화물·단백질·지방)에 해당하는 음식`, `먹은 양`, `포만감` 등을 입력하며
...
데이터가 누적될수록 **조회 성능 저하가 발생할 가능성**이 있는 구조
...
>  
> HabitSnap은 단계별 성능 개선 과정을 통해 조회 효율을 높이고, 
...
**문제**  
수정 API에서 어떤 값이 수정되고, 어떤 값이 유지되는지 기준이 모호한 상황

**해결**  
PATCH의 목적에 맞게  
→ *요청으로 전달된 필드만 수정하고, 전달되지 않은 필드는 그대로 유지*하도록 정책을 정함
식별자(id)는 요청 바디가 아닌 PathVariable로만 전달하도록 구조를 수정함
...
문서가 복잡해지고 유지보수가 어려워짐

**해결**  
Swagger의 자동 생성 기능을 기본으로 사용하고,  
요청/응답 예시와 꼭 필요한 설명만 추가함
...
문서 가독성과 프론트엔드 사용성이 떨어짐

**해결**  
응답 DTO에서 시간 값을 문자열(`HH:mm:ss`)로 변환해 반환하도록 수정함
...
**문제**  
`@CacheEvict` 설정 중 SpEL 표현식에서 null 에러가 발생함

**해결**  
SpEL이 파라미터 이름을 인식하지 못하는 문제임을 확인하고,  
파라미터 인덱스 기반 접근 방식으로 수정함
...
- 기능 구현보다 **왜 이렇게 설계했는지**를 고민하는 습관을 갖게 되었습니다.
- API는 단순히 동작하는 것보다
**사용자가 어떻게 쓰는지와 그 규칙을 명확히 하는 것이 더 중요하다고 인식**하게 되었습니다.
- 인증, 성능, 문서화를 포함한 **백엔드 개발 전체 흐름 경험**해봤습니다.
- 사용자별·날짜별 조회 패턴을 분석해 **인덱스 설계·쿼리 최적화·캐싱 적용**의 효과를 직접 검증해봤습니다.
- 단순한 코드 개선을 넘어 **DB 접근 효율과 응답 속도를 함께 향상시키는 설계 사고**를 배웠습니다.

---

## 📌 한 줄 요약
> HabitSnap은 식사 기록이라는 작은 도메인을 통해  
> 백엔드 개발자가 마주치는 설계와 문제 해결 과정을 직접 고민하고 구현한 프로젝트입니다.
